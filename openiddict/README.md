...
If you are adding authentication to a new project, the easiest way to provide this object is by defining it in the appsettings.json file (example below). However if you are replacing IdentityServer4 in an existing project then it is generally simpler to leave the configuration file as-is and to convert from the existing structure to `OpenIdConnectConfig` in code. If you are seeding configuration data in your deployment pipeline, as described [here](#seeding-clients-and-scopes-in-the-database), and are converting the config from an existing structure, then you will need to provide an implementation of `IOpenIdConnectConfigMapper` which should contain the conversion code. However as long as your configuration section is named `"OpenIdConnectConfig"` and can be mapped directly to an `OpenIdConnectConfig` object then you don't need to do any additional work and the seeding process with automatically pull the configuration in.
